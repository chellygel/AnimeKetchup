...
def run():
    ...
    end_date_prompt = (
        "What date would you like to have completed this series?(Ex: 15 Aug 2050):\n"
    )
    ...
    per_day_limit = (
        input("What is the limit of episodes you will watch per day? [0+] ") or 0
    )
    ...
    if watch_schedule["is_possible"]:
        ...
        if watch_schedule["extra_episodes"]:
            print(
                f"You will have {watch_schedule['extra_episodes']} extra, "
    ...
    watch_schedule = wsb.calculate_watch_schedule(
        episodes, end_date, start_date, per_day_limit
    )
    print(f"Wow, that's only {watch_schedule['days']} days between today and then !!!")
    ...
    if watch_schedule["is_possible"]:
        print(
            ...
            "to complete the show on time!"
        )
        if watch_schedule["extra_episodes"]:
            print(
                f"You will have {watch_schedule['extra_episodes']} extra,"
                "you can add an additional watch day or tack them on to one of your other days."
            )
    else:
        print("I'm sorry, but that limit and end date make it impossible!")
        print("Don't worry, consider these options instead:")
        print(
    ...
    print(
        ...
        print(
            f"Option 2: You could also choose to watch {per_day_limit} episodes per day"
            "and finish by {watch_schedule['realistic_end_date']}"
        )

    ff = fun_fact.random_factoid()
    ff_watch_schedule = wsb.calculate_watch_schedule(
        ff[1], end_date, start_date, per_day_limit
    )

    print(
        f"In comparison you would have to watch {ff_watch_schedule['base_eps_per_day']} episodes per day "
        "if you were watching {ff[0]}!"
    )
    print(" ")
    print(" ")
    print(LUFFY)
